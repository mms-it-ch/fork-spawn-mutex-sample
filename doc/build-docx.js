...
const body = [
  ...
  new Paragraph({ spacing: { after: 480 }, border: { bottom: { style: BorderStyle.SINGLE, size: 8, color: '534AB7', space: 8 } },
    children: [new TextRun({ text: 'Programmdokumentation · Stand 22.09.2026', color: '555555' })] }),
  ...
  P('Der z/OS-spezifische Teil ist nicht auf einem z/OS-System getestet: der `spawnp()`-Zweig mit `fd_map` und `struct inheritance`, die Feature-Test-Makros, das Makefile mit `xlclang` oder `xlc` und das JCL. Beim ersten Übersetzen auf z/OS ist deshalb mit kleinen Anpassungen zu rechnen, zum Beispiel bei den Makros oder Compiler-Optionen.'),
  ...
  table([3400, 6238], ['Umgebung', 'Aufruf'], [
    ['z/OS, xlclang (Voreinstellung)', '`make`'],
  ...
  SPACER(),
  P('Voreingestellt ist `xlclang` aus z/OS XL C/C++ V2.4.1 mit vollem Pfad `/usr/lpp/cbclib/xlclang/bin/xlclang` und den Optionen `-q64 -O2`. `xlclang` erzeugt nur 64-Bit-Code, deshalb `-q64`. Für `xlc` ist zusätzlich `-qlanglvl=extc99` nötig, das unter anderem `snprintf()` sichtbar macht. Eine eigene Thread-Bibliothek (`-lpthread`) braucht z/OS nicht. Das Makefile enthält bewusst keine Tabulatorzeichen: Das Kommando steht mit `;` in derselben Zeile wie die Regel, zum Beispiel `clean: ; rm -f $(PROG) *.o`.'),
  H2('7.2 Compiler-Bibliothek nicht in LNKLST'),
  P('Der Compiler-Treiber `/usr/lpp/cbclib/xlclang/exe/clcdrvr` ist kein Programm, sondern ein externer Link auf das MVS-Modul `CLCDRVR`. Findet z/OS dieses Modul nicht in STEPLIB, LNKLST oder LPA, bricht die Übersetzung ab. Bei `xlc` lautet die Meldung `FSUM3221 Cannot spawn program …/ccndrvr`, bei `xlclang` `FSUM3224 … clcdrvr: signal 9 received`. Im SYSLOG steht dann ein Abend `EC6` mit Reason Code `….C032` und `BPXP018I`; die letzten vier Stellen `C032` bedeuten, dass ein Sticky-Programm nicht im MVS-Suchpfad gefunden wurde. Register 5 im Symptom-Dump enthält den Modulnamen in EBCDIC.'),
  P('Die Shell-Variable `STEPLIB` hilft nicht. Der Compiler liest die Bibliothek ausschließlich aus dem Attribut `steplib` seiner Konfigurationsdatei. Prüfen Sie zuerst, was dort steht:'),
  ...codeBlock('grep -n steplib /usr/lpp/cbclib/xlclang/etc/xlclang.cfg'),
  SPACER(),
  P('Steht dort eine Bibliothek, die das Modul `CLCDRVR` enthält, ist nichts zu tun. Steht dort `NONE` oder eine falsche Bibliothek, erzeugt `make cfg CMPLIB=…` eine lokale Kopie `cc.cfg` mit dem richtigen Namen, und übersetzt wird mit `make CFG=-F./cc.cfg`. Die Option `-F` erwartet den Pfad direkt angehängt, ohne Leerzeichen. Welche Bibliotheken es gibt, zeigt `tso "LISTCAT LEVEL(CBC)"`; typische Namen sind `CBC.SCLCCMP` für `xlclang` und `CBC.SCCNCMP` für `xlc`. Ob ein Modul enthalten ist, zeigt `tso "LISTDS \'CBC.SCLCCMP\' MEMBERS"`. Für `xlc` kommt beim `make cfg` zusätzlich `CCCFG=/usr/lpp/cbclib/xlc/etc/xlc.cfg` dazu.'),
  ...
  P('`USSDEMO.jcl` enthält zwei BPXBATCH-Steps. `COMPILE` wechselt in das Quellverzeichnis, zeigt mit `grep` die `steplib`-Zeile der Compiler-Konfiguration im Job-Output an und ruft dann `make clean all` auf. Muss eine eigene `cc.cfg` verwendet werden, ersetzen Sie die beiden Zeilen wie im JCL-Kommentar beschrieben. `RUN` startet das Programm mit `_BPX_SHAREAS=YES` und läuft nur, wenn `COMPILE` mit RC 0 geendet hat. BPXBATCH liefert den Exit-Code der Shell mit 256 multipliziert zurück, ein fehlgeschlagenes `make` erscheint also als RC 256 oder höher. Stdout und stderr beider Steps gehen in den Job-Output.'),
  ...
  BULLET('Für einen anderen Compiler als `xlclang` hängen Sie `CC=…` und `CFLAGS=…` an den `make`-Aufruf an.'),
