...
const body = [
  ...
  P('`USSDEMO.jcl` enthält zwei BPXBATCH-Steps. `COMPILE` wechselt in das Quellverzeichnis, erzeugt mit `make cfg` die lokale Compiler-Konfiguration, zeigt deren `steplib`-Zeile per `grep` im Job-Output an und ruft dann `make clean all CFG=-F./cc.cfg` auf. `RUN` startet das Programm mit `_BPX_SHAREAS=YES` und läuft nur, wenn `COMPILE` mit RC 0 geendet hat. BPXBATCH liefert den Exit-Code der Shell mit 256 multipliziert zurück, ein fehlgeschlagenes `make` erscheint also als RC 256 oder höher. Stdout und stderr beider Steps gehen in den Job-Output.'),
